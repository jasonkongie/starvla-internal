...
class Qwen_GR00T(baseframework):
    ...
    def predict_action(
        ...
        **kwargs,
        ...
        Args:
            profiler: Optional InferenceProfiler. If provided, times preprocessing,
                      vlm_backbone, action_head, and total phases. Pass None (default)
                      for zero overhead on normal inference.
        ...
        # Step 1: QWenVL input format (preprocessing: tokenization + image encoding prep)
        if profiler is not None:
        ...
        # Step 3: Action head (DiT flow-matching denoising)
        if profiler is not None:
